...
export namespace Utils {
  ...
  export const typeRegex: RegExp = /^\[object\s(.*)]$/;
  ...
  export function bindAll(object) {
    getAllPropertyNames(object).forEach((property) => {
   ...
   */
  export function getAllPropertyNames(object: any): Array<string> {
    let props = {};
   ...
   *
   * @param {*} object The object to bind all the functions for.
   * @param {...string} fields A varargs list of function names to bind.
   */
  export function bindSome(object: any, ...fields: string[]) {
    if (fields.length > 0) {
      for (let i = 0; i < fields.length; i++) {
        const func = object[fields[i]];
        if (!isDefined(func) || !(func instanceof Function)) {
          throw new TypeError('The object does not contain a function named [' + fields[i] + ']');
        }

        object[fields[i]] = func.bind(object);
      }
    }
   ...
   */
  export function bindSafe(func: Function, context: object): Function {
    if (!isDefined(func)) {
   ...
   */
  export function calculateTextLength(element: PrimeElement, text: string): number {
   ...
   */
  export function callIteratively(totalDuration: number, timesToCall: number, stepFunction: Function, endFunction?: Function) {
   ...
   */
  export function convertStyleName(name: string): string {
    if (name === 'float') {
      return 'cssFloat';
    }
    ...
    while (dash !== -1) {
      ...
      if (start === 0) {
        ...
        result = result.concat(capitalize(piece));
      }
    ...
    return result + capitalize(name.substring(start));
  }
   ...
   */
  export function dataSetToOptions(element: PrimeElement): object {
    const options = {};
   ...
   * @returns {string}
   */
  export function leftPadNumber(number: number, width: number): string {
    const sign = number < 0 ? '-' : '';
   ...
   */
  export function parseCSSMeasure(measure: string): number {
   ...
   *
   * @param {Array<T>} fromArray The array to remove from.
   * @param {Array<T>} toRemove The values to remove.
   * @template T The type in the arrays
   */
  export function removeAllFromArray<T>(fromArray: Array<T>, toRemove: Array<T>) {
    for (let i = 0; i < toRemove.length; i++) {
      removeFromArray(fromArray, toRemove[i]);
    }
  }
   ...
   * @template T
   */
  export function removeFromArray<T>(array: Array<T>, obj: T) {
  ...
  /**
   * Helper function to provide a one liner to behave as if you returned 'false' from a legacy version of Prime.ts.
   *
  ...
  export function type(object) {
    return Object.prototype.toString.call(object).match(typeRegex)[1];
  }
}
